...
-- 2) 정렬된 테이블을 TMP라 명명하고 여기에 rownum rn을 부여
select TMP.*, rownum rn from (select * from employees order by salary desc)TMP;
-- 3) 정렬, rownum rn 부여가 끝난 테이블에서 rn이 1 이상 3 이하인 값을 성, 이름, 월급을 불러오기
...
-- 3) 정렬, rownum rn 부여가 끝난 테이블에서 rn이 1 이상 3 이하인 데이터의 성, 이름, 고용일을 불러오기
...
-- 3) 정렬, rownum rn 부여가 끝난 테이블에서 rn이 1인 데이터의 성, 이름,  불러오기

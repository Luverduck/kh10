...
// 		});
	...
	$(function(){
		// checkbox
		$("input[name=type][value=${paramValues.type[0]}]").prop("checked", true); // name이 type이고 value가 사탕인 input 태그의 값 체크
		$("input[name=type][value=${paramValues.type[1]}]").prop("checked", true);
		$("input[name=type][value=${paramValues.type[2]}]").prop("checked", true);
		$("input[name=type][value=${paramValues.type[3]}]").prop("checked", true);
		
		// select
		$("select[name=sort]").eq(0).val("price desc");
	});
